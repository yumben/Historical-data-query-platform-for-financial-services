	...
	$.jsonp4Table = function(options) {
		...
		this.drewTable = function(_params) {
			...
			ajaxSend(opt.action, params, function(data) {
				...
				opt.totalPage = data.total % opt.pageSize == 0 ? data.total / opt.pageSize : Math.ceil(data.total / opt.pageSize);
				opt.totalCount = data.total;
				$table.next().remove();
