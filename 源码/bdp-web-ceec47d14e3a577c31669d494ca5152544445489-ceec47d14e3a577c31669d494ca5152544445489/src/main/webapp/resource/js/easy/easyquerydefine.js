...
$(function() {	
	...
	function selectLayer(obj) {
		...
		ajaxSend('/bdp-web/metadata/selectObjAndPropertie', param, function(data){
			//console.log(JSON.stringify(data)+'全选字段的表字段!!!')
			var list = data.list;
...
$(document).on('click','.editScreen',function(){
	...
	if(isNoemty(dynamic_field) !=''){
		...
		ischeckDown = 'style="display:inline-bloc"';
	}
	...
	if(fieldType == 'pk'){
		...
		tdHtml.append('<input type="text" class="form-control value1 datepicker dateStar" autocomplete="off" name="value1" value="'+value1+'">');
		if(cond == 'between'){
