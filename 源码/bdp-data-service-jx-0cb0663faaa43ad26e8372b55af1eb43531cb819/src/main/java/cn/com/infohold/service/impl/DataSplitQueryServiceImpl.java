...
@Service
@Log4j2
public class DataSplitQueryServiceImpl implements IDataSplitQueryService {
...
//		projectObj.put("count", "$count");
	...
	@Override
	public RetBean queryDataList(QueryBean queryBean) throws Exception {
		Map<String, Object> dataBaseInfoMap = getDbInfo(queryBean);
		...
		try {
			...
			totalMap.put("pageTotal", getPageTotal(queryBean, result.getResults()));
			result.setTotalResult(totalMap);
		} catch (Exception e) {
			result.setRet_code("-1");
			result.setRet_message(e.getLocalizedMessage());
			e.printStackTrace();
	...
	@Override
	public RetBean queryTotalCount(QueryBean queryBean) throws Exception {
		Map<String, Object> dataBaseInfoMap = getDbInfo(queryBean);
		Integer count = 0;
		log.debug(" start query data count");
		RetBean result = new RetBean();
	...
	@Override
	public RetBean queryTotal(QueryBean queryBean) throws Exception {
		Map<String, Object> dataBaseInfoMap = getDbInfo(queryBean);
		RetBean retBean = new RetBean();
		long startTime = System.currentTimeMillis();
		long endTime = System.currentTimeMillis();
		...
		Map<String, Object> retmap = getTotalMap(queryBean, mongoClient, table, filter, sorts, isGroup, pageTotal);
		retBean.setTotalResult(retmap);
		return retBean;
	}

	public Map<String, Object> getDbInfo(QueryBean queryBean) throws Exception {
		String metadataTableCode = queryBean.getTable();
		MetaData metadata = MetadataUtil.getMetadataBymetadataCode(metadataTableCode, "p"); // 元数据库表属性信息
		RetBean result = new RetBean();
		if (null == metadata) {
