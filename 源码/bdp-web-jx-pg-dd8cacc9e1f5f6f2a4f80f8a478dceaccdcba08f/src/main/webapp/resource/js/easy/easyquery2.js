...
$(function() {
	...
	var token =window.parent.document.getElementById("hidToken").val();
	var param = {
		'id' : templateId,'token':token
	}
	ajaxSendNotoken('/bdp-web/easyQuery/selectEasyQueryTemplate', param, successCallBackFun);
	function successCallBackFun(data) {
		...
		//外键pk
		var metadataIdVal = {
			'metadata_id' : '','token':token
		};
		if (conditions != undefined && conditions != 'undefined' && conditions != null && conditions != 'null' && conditions != '') {
		...
		//数据筛选	
		ajaxSendNotoken('/bdp-web/metadata/selectMetadataPK', metadataIdVal, function(pklist) {
			if (conditions !='' && conditions !='null' && conditions !='undefined' && conditions !=null && conditions !=undefined) {
				$("#conditions").show();				
				$.each(conditions, function(index, el) {
					...
					if (typeField == 'pk') {
						...
						if(cond == 'between'){
							elem.append('<b>~</b>');
							elem.append('<div class="input_close"><input type="text" class="countVal1 form-control datepicker dateEnd" value="'+valule2+'">'
									+'<i class="closeInput"></i></div>');
						}
						elem.append(button);
						intDate();
						...
						if(cond == 'between'){						
							elem.append('<b>~</b>');
							elem.append('<div class="input_close"><input type="text" class="countVal1 form-control" value="'+valule2+'" ><i class="closeInput"></i></div>');
						}
						elem.append(button);
						var coundVal = seleTime.find('option');
						$.each(coundVal,function(){
	...
	$('.dataSelect').click(function() {
		...
		//var conspanText = _this.siblings('span').text();
		...
		layer.open({
			...
			yes : function(index, layero) {
				...
				var bewttHtml = $('<b>~</b> <div class="input_close">'
	...
	function paramJson(valJson) {
		...
		var valis_summary = $('input[name="is_summary"]').val();
		...
		$.each(dimenField, function(index, el) {
			...
			if (val == '') {
				...
				//dimeListTd['alias'] = val;
				dimeListTd['thId'] = thId;
				dimeJsonTd.push(dimeListTd);
			}
		})	
		//console.log(JSON.stringify(dimeJsonTd)+'!@@@@')
		viewTh(dimeJsonTd, thField, orderArr,thTitleLi);
		var initCheck = $('#filedText > div input[type="checkbox"]');
		var initCheckArry = [];
		var isThTrue = false;
		getafield(initCheckArry, initCheck, isThTrue);
		viewTh(initCheckArry, thField, orderArr,thTitleLi);
		//表头的宽度计算赋值		
	...
	function selectData() {
		...
		var params = {
			"param" : JSON.stringify(paramSearch),'token':token
		};
		//console.log(JSON.stringify(paramSearch) + '查询参数')
		ajaxSendNotoken('/bdp-web/easyQuery/selectEasyqueryData', params, function(data) {
			//console.log(JSON.stringify(data)+'%^&&')
			var list = data.results;
			total = data.total; //总记录数
			totalPage = data.totalPage; //总页数	
			listTdData = list; //返回的数据			
			$('#tdData').html('');
			var ths = $("#thFiled").find("th");
			if (list && list.length > 0) {						
				var tdObj = $('#tdData');					
				tdDataHtml(list, ths, tdObj);
				//小计
				if (data.totalResult) {
					totalResult = data.totalResult; //合计字段节点
					fieldTotal = totalResult.total; //总合计		
					pageTotal = totalResult.pageTotal; //当页合计
					if (!(JSON.stringify(totalResult) == "{}")) {
						...
						if (fieldTotal) {
							var flag = false;
							totalHtml(tdObj, fieldTotal, ths, flag);
						}
					}
				}
				...
				var tdData = $('#tdData');
				tdData.append('<tr><td colspan="' + ths.length + '" style="text-align:center;font-size:16px;">查询无相关数据！</td></tr>')
				//表头的宽度计算赋值	
				var thTitleLi = $('#thTitle');
				var tbodyCon = $('#tbodyCon');
				var li = thTitleLi.find('li');
...
function viewTh(arr, obj, orderArr,liObj) {
	...
	$.each(arr, function(index, el) {
		...
		if ('asc' == el.orderType && orderArr.length == 0) {
			...
			orderArr.push(orderJson);
			thHead = $('<li name="' + el.alias + '" is_compute="'+el.is_compute+'" code="' + el.fieldData + '" id="' + el.thId + '" sort="asc" field_type="' + el.data_type + '" show_form="' + el.show_form + '" field_unit_limit="' + el.field_unit_limit + '" field_decimal_point="' + el.field_decimal_point + '">' + text + '</li>');
...
function tdDataHtml(data, thElem, obj) {
	$.each(data, function(index, el) {
		...
		$.each(thElem, function(i, elTh) {
			...
			var tdValue = el[key];
			if (tdValue == 'undefined' || tdValue == undefined || tdValue == 'null' || tdValue == null) {
				tdValue = ''
			} else {
				tdValue = el[key];
			}
			//单位换算
//			tdValue = unitChange(field_unit_limit, unit, tdValue);
			//千分位
...
function totalHtml(obj, countData, thData, flag) {
	...
	$.each(thData, function(index, el) {
		...
		//单位换算
		value = unitChange(field_unit_limit, unit, value);
		//千分位
