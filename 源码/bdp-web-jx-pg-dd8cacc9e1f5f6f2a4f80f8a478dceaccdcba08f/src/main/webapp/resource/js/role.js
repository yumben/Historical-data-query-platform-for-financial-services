$(function(){
	...
	var _json4Table = new $.json4Table({
		action : "/bdp-web/userManager/roleQuery",
	...
	function operateFormatter(id) {
		return [
		    '<a class="edit" href="javascript:roleAuth(\'' + id
			+ '\')" title="权限设置">',
...
function callBack(data){
	...
	ajaxSend('/bdp-web/authorization/resourceQuery',{"param":"{}"},function(resource){
		...
		$.each(obj,function(index,el){
			...
			//console.log(JSON.stringify(resourceOperation)+'666');
		...
		layer.open({
			...
			yes : function(index, layero) {				
				...
				console.log(JSON.stringify(param)+'333')
