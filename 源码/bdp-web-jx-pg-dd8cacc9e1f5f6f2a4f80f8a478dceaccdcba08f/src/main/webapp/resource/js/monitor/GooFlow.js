...
function GooFlow(bgDiv, property) {
	...
	if (this.$editable) {
		this.$workArea.on("click", {
			...
			This.addNode(getUuid(), {
				name : "_node_" + This.$max,
				left : X,
				top : Y,
				type : This.$nowType,
				actionId : ''
...
GooFlow.prototype = {
	...
	addNode : function(id, json) {
		...
		if (json.type.indexOf(" round") < 0) {
			...
			if (navigator.userAgent.indexOf("8.0") != -1)
				hack = 2;
			this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' actionId='" + json.actionId + "' style='top:" + json.top + "px;left:" + json.left + "px' status='" + json.status + "'><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td><td>" + json.name + "</td><td class='icostuat'></td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		//this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' actionId='"+json.actionId+"' style='top:"+json.top+"px;left:"+json.left+"px' status='"+json.status+"'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td><td>"+json.name+"</td><td class='icostuat'></td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		} else {
			json.width = 35;
			json.height = 35;
			this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px' status='" + json.status + "'><table cellspacing='0'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>" + json.name + "</div></div>");
		}
	...
	initWorkForNode : function() {
		...
		this.$workArea.delegate(".GooFlow_item", "mouseup", {
			...
			if (lineStart && !This.$mpTo.data("p")) {
				...
				var getUuid = function() {
					var dtCur = new Date().getTime();
					return 'trasition_' + dtCur;
				}
				This.addLine(getUuid(), {
					from : lineStart.id,
					to : this.id,
					name : "",
					condition : ""
				});
				//This.addNode(getUuid(),{name:"_node_"+This.$max,left:X,top:Y,type:This.$nowType});
	...
	focusItem : function(id, bool) {
		...
		if (jq.prop("tagName") == "DIV") {
			...
			if (GooFlow.prototype.useSVG != "") {
				...
				jq[0].strokeColor = GooFlow.prototype.color.mark || "#ff3300";
			if (!this.$editable) return;
			var x,
				y,
				from,
